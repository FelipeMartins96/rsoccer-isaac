...
def get_sim_params():
    ...
    p.gravity = gymapi.Vec3(
        0.0, 0.0, -9.8
    )  # default gymapi.Vec3(0.000000, -9.800000, 0.000000)
...
def add_ball(group=0):
    ...
    ball_actor = gym.create_actor(
        env=env, asset=ball_asset, pose=initial_ball_pose, group=group, filter=0b01
    )
...
def add_robot(group=0):
    ...
    rbt_asset = gym.load_asset(
        sim=sim, rootpath='./assets/', filename='vss_robot.urdf', options=options
    )
    ...
    actor = gym.create_actor(
        env=env, asset=rbt_asset, pose=rbt_pose, group=group, filter=0b00, name='robot'
    )
...
def add_field(group=0, filter=0b10):
    ...
    def add_side_walls():
        ...
        for dirX, dirY in swDirs:
            ...
            swActor = gym.create_actor(
                env=env, asset=swAsset, pose=swPose, group=group, filter=filter
            )
    ...
    def add_end_walls():
        ...
        for dirX, dirY in ewDirs:
            ...
            ewActor = gym.create_actor(
                env=env, asset=ewAsset, pose=ewPose, group=group, filter=filter
            )
    ...
    def add_goal_walls():
        ...
        for dirX, dirY in gwDirs:
            ...
            gwActor = gym.create_actor(
                env=env, asset=gwAsset, pose=gwPose, group=group, filter=filter
            )
